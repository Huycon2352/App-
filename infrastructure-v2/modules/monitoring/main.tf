locals {
  discovered_remote_write_host = trimspace(var.remote_write_host) != "" ? var.remote_write_host : format(
  ...
  remote_write_url = format(
    ...
    local.discovered_remote_write_host,
  ...
  agent_environment = {
    ...
    PROMETHEUS_REMOTE_WRITE_HOST      = local.discovered_remote_write_host
